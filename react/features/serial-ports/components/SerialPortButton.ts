...
class SerialPortButton extends AbstractButton<IProps> {
    ...
    // Only used when worker startup fails (before transfer).
    ...
    private _onWorkerMessage = (msg: IWorkerToHostMessage) => {
        if (msg.radioId !== SerialPortButton.RADIO_ID) {
            return;
        }

        if (msg.type === 'log') {
            _trace(SerialPortButton.RADIO_ID, msg.message, msg.extra);
            return;
        }

        if (msg.type === 'status') {
            this.props.dispatch(setSerialPortStatus(SerialPortButton.RADIO_ID, msg.status));
            return;
        }

        if (msg.type === 'connected') {
            this.props.dispatch(setSerialPortConnected(SerialPortButton.RADIO_ID, msg.connected));
            return;
        }
    ...
    private async connectSerialPort() {
        try {
            // Tear down previous active instance if overflow menu remounted.
            ...
            this.bridgeClient = new SerialBridgeClient(SerialPortButton.RADIO_ID, this._onWorkerMessage);

            const workerConfig: ISerialBridgeStartConfig = {
                radioId: SerialPortButton.RADIO_ID,
                wsUrl: `wss://station.wu2x.com:8443/webRRL/websocket2?clientIp=${encodeURIComponent(clientIp)}`,
            ...
            // Ownership transferred to worker; keep this.port only for failure cleanup.
            this.port = null;

            _trace(SerialPortButton.RADIO_ID, 'connectSerialPort: started worker');
        } catch (e) {
            const radio = this.props.t(this.label) as unknown as string;
    ...
    private async disconnect({ clearActiveInstance = true }: { clearActiveInstance?: boolean; } = {}) {
        _trace(SerialPortButton.RADIO_ID, 'disconnect: requested');
        ...
        // Ask worker to shutdown and then terminate it.
