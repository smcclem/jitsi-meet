...
class SerialPortButtonK3Radio2 extends AbstractButton<IProps> {
    ...
    private _onWorkerMessage = (msg: IWorkerToHostMessage) => {
        if (msg.radioId !== SerialPortButtonK3Radio2.RADIO_ID) {
            return;
        }

        if (msg.type === 'log') {
            _trace(SerialPortButtonK3Radio2.RADIO_ID, msg.message, msg.extra);
            return;
        }

        if (msg.type === 'status') {
            this.props.dispatch(setSerialPortStatus(SerialPortButtonK3Radio2.RADIO_ID, msg.status));
            return;
        }

        if (msg.type === 'connected') {
            this.props.dispatch(setSerialPortConnected(SerialPortButtonK3Radio2.RADIO_ID, msg.connected));
            return;
        }
    ...
    private async connectSerialPort() {
        try {
            ...
            this.bridgeClient = new SerialBridgeClient(SerialPortButtonK3Radio2.RADIO_ID, this._onWorkerMessage);

            const workerConfig: ISerialBridgeStartConfig = {
                radioId: SerialPortButtonK3Radio2.RADIO_ID,
                wsUrl: `wss://station.wu2x.com:8443/webRRL/k3s/radio2/websocket?clientIp=${encodeURIComponent(clientIp)}`,
            ...
            _trace(SerialPortButtonK3Radio2.RADIO_ID, 'connectSerialPort: started worker');
        } catch (e) {
            const radio = this.props.t(this.label) as unknown as string;
    ...
    private async disconnect({ clearActiveInstance = true }: { clearActiveInstance?: boolean; } = {}) {
        _trace(SerialPortButtonK3Radio2.RADIO_ID, 'disconnect: requested');
